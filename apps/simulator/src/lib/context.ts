...
/**
 * One options object that satisfies both consumers: raw `new IORedis(opts)` and
 * bullmq's `{ connection }`. bullmq 6 stopped depending on ioredis and ships its
 * own structural RedisOptions, so the two types are no longer the same type.
 */
export type RedisOptions = IORedisOptions & BullRedisOptions;
