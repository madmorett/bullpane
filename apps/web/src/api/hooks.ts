...
export const qk = {
  ...
  allSchedulers: (cid: string, p: AllSchedulersParams) => ["connections", cid, "schedulers", p] as const,
  jobTree: (cid: string, q: string, id: string, maxNodes: number) =>
    ["connections", cid, "queue", q, "job", id, "tree", maxNodes] as const,
...
export type QueueActionInput =
  | { action: "resume" | "obliterate" }
  | { action: "pause"; body: PauseQueueInput }
...
export function useQueueToggle(cid: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ queue, action, reason }: { queue: string; action: "pause" | "resume"; reason?: string }) =>
      api.post<{ ok: boolean }>(`${queuePath(cid, queue)}/${action}`, action === "pause" ? { ...(reason ? { reason } : {}) } : undefined),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: qk.connection(cid) });
    },
  });
...
export interface AllSchedulersParams {
  q?: string;
  withinMs?: number;
  limit?: number;
}
...
/**
 * Every scheduler of a connection, across queues. The server only reads queues
 * that report schedulersCount > 0, so this is cheap even on a big install.
 */
export function useAllSchedulers(cid: string | undefined, params: AllSchedulersParams = {}) {
  return useQuery({
    queryKey: qk.allSchedulers(cid ?? "", params),
    queryFn: () => api.get<ConnectionSchedulersPage>(`/connections/${seg(cid!)}/schedulers`, { query: { ...params } }),
    enabled: !!cid,
    refetchInterval: poll(POLL.queues),
    placeholderData: keepPreviousData,
  });
...
/**
 * The parent/child tree of one flow instance. Polls while anything is unfinished
 * and stops once the whole tree has settled, the same rule useJob follows.
 */
export function useJobTree(cid: string | undefined, queue: string | undefined, jobId: string | undefined, maxNodes = 200) {
  return useQuery({
    queryKey: qk.jobTree(cid ?? "", queue ?? "", jobId ?? "", maxNodes),
    queryFn: () => api.get<JobTree>(`${queuePath(cid!, queue!)}/jobs/${seg(jobId!)}/tree`, { query: { maxNodes } }),
    enabled: !!cid && !!queue && !!jobId,
    refetchInterval: (query) => {
      if (typeof document !== "undefined" && document.hidden) return false;
      const d = query.state.data;
      if (d && d.nodes.every((n) => n.finishedOn !== null || n.missing)) return false;
      return POLL.job;
    },
    placeholderData: keepPreviousData,
  });
