...
export function ConnectionPage() {
  ...
  const [pausing, setPausing] = useState<QueueEntry | null>(null);
  ...
  // Resume is the undo, so it fires straight away. Pause opens the dialog that
  // asks for a reason (see PauseQueueDialog).
  const onToggle = isOperator
    ? (e: QueueEntry) => {
        ...
        toggle.mutate({ queue: e.queue.name, action: "resume" }, { onSuccess: () => toast.success(`${e.queue.name} resumed`), onError: (err) => toast.error(errorMessage(err)) });
      }
    : undefined;
  const confirmPause = (reason: string | undefined) => {
    if (!pausing) return;
    const name = pausing.queue.name;
    toggle.mutate(
      { queue: name, action: "pause", ...(reason ? { reason } : {}) },
      {
        onSuccess: () => {
          setPausing(null);
  ...
  return (
    <Page wide>
      ...
      <PauseQueueDialog queue={pausing?.queue.name ?? null} onClose={() => setPausing(null)} onConfirm={confirmPause} loading={toggle.isPending && toggle.variables?.action === "pause"} />
    </Page>
  );
}
