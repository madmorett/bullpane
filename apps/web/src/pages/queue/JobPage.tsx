  ...
  return (
    <Page>
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 flex items-center gap-1 font-mono text-xs text-fg-muted">
            ...
            <CopyButton value={() => safeJsonStringify(d)} label="Copy JSON" copiedLabel="Copied JSON" showLabel size="xs" icon={<Braces />} className="ml-1 font-sans" />
          </p>
        </div>
        {isOperator && (
          <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="mb-4 grid gap-3 lg:grid-cols-3">
        ...
        {/* The counters above say HOW MANY children are pending; the tree says WHICH.
            Only offered when this job is actually part of a flow. */}
        {(d.parent || d.dependencies) && (
          <div className="mt-3 border-t border-border pt-3">
            <Link
              to={routes.jobTree(connectionId, queue, d.id)}
...
/**
 * When a delayed job becomes runnable, decoded from its zset score on the
 * server. The hash's `delay` field cannot answer this after a backoff retry, and
 * an operator looking at a delayed job wants a clock time, not "60s".
 */
function RunsAt({ at }: { at: number | null }) {
  if (!at) return <>–</>;
  return (
    <span className="flex flex-col leading-tight">
      <RelativeTime value={at} />
      <span className="font-mono text-[10px] text-fg-subtle">{formatDateTimeMs(at)}</span>
    </span>
  );
}
        ...
        {steps.map((s, i) => (
          <li key={s.label} className={cn("relative z-[1] flex flex-1 flex-col", i === 1 ? "items-center text-center" : i === 2 ? "items-end text-right" : "items-start")}>
            ...
            <span className="text-[11px] text-fg-muted">
              {s.ts ? <RelativeTime value={s.ts} /> : job.state === "active" && i === 2 ? "running…" : job.state === "delayed" && i === 1 && job.delayedUntil ? <>runs <RelativeTime value={job.delayedUntil} /></> : "–"}
            </span>
            {s.ts && <span className="font-mono text-[10px] text-fg-subtle">{formatDateTimeMs(s.ts)}</span>}
            {!s.ts && job.state === "delayed" && i === 1 && job.delayedUntil && <span className="font-mono text-[10px] text-fg-subtle">{formatDateTimeMs(job.delayedUntil)}</span>}
          </li>
        ))}
      </ol>
