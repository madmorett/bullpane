    ...
    <>
      <tr className={cn(failed && "bg-danger/5")}>
        ...
        <Td>
          ...
          {/* A stated reason (pause) is the one detail worth reading without expanding the row. */}
          {typeof entry.detail?.reason === "string" && entry.detail.reason && (
            <p className="mt-0.5 max-w-md truncate text-[11px] text-fg-muted italic" title={entry.detail.reason}>
              &ldquo;{entry.detail.reason}&rdquo;
            </p>
          )}
        </Td>
        <Td>
