...
export function JobsTable({
  ...
  // The delayed tab has nothing to say under "Finished"; what matters there is when each job runs.
  const allDelayed = visibleCount > 0 && jobs!.every((j) => j.state === "delayed");
      ...
      <thead>
        <tr>
          ...
          */}
          ...
          <Th className="w-28">{allDelayed ? "Runs at" : "Finished"}</Th>
...
function JobRow({
  ...
  return (
    <Tr onActivate={onOpen} selected={selected} className={cn(pending && "opacity-50")}>
      ...
      <Td muted>
        {job.state === "delayed" && job.delayedUntil ? (
          // Decoded from the delayed zset score on the server: the clock time
          // the job becomes runnable, which the hash's `delay` cannot tell after a retry.
          <Tooltip content={<>runs at {formatDateTime(job.delayedUntil)}</>} side="left">
            <span className="text-state-delayed">
              <RelativeTime value={job.delayedUntil} />
            </span>
          </Tooltip>
        ) : (
