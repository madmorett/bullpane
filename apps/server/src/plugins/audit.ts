...
const ROUTE_ACTIONS: Record<string, AuditAction> = {
  ...
  // Bulk actions. Separate routes (and separate actions in the log) on purpose:
  // "retried 50 jobs" and "retried 1 job" are different events to an auditor,
  // and the handler's `detail` carries requested/ok/failed counts.
