import {
  ...
  listConnectionSchedulersQuerySchema,
...
export async function connectionRoutes(app: FastifyInstance): Promise<void> {
  ...
  /*
    Registered BEFORE `/connections/:id` on purpose: Fastify would otherwise be
    free to match "reorder" as an :id and answer 404 for a valid request.

    `operator`, not `admin`: rearranging the sidebar is the same weight as hiding
    a queue — it changes what the team looks at and destroys nothing. Requiring
    an admin for it would mean nobody reorganises anything during an incident.
  */
  app.put("/connections/reorder", { preHandler: [operator, blockInDemo] }, async (request): Promise<RedisConnection[]> => {
    const { ids } = reorderSchema.parse(request.body);
    const list = await app.ctx.connections.reorder(ids);
    request.auditDetail({ count: ids.length });
    return list;
  });
  ...
  /**
   * Every job scheduler of the connection, across all queues.
   *
   * Schedulers are invisible in the 8 job states, and per-queue tabs mean nobody
   * ever sees what the system as a whole is scheduled to do. Free edition: the
   * per-queue scheduler list already is.
   *
   * Cost is driven by `schedulersCount` from the stats read the sidebar makes —
   * queues with no schedulers are never touched. See listAllSchedulers.
   */
  app.get<IdParams>("/connections/:id/schedulers", { preHandler: [viewer] }, async (request): Promise<ConnectionSchedulersPage> => {
    const query = listConnectionSchedulersQuerySchema.parse(request.query);
    const row = await app.ctx.connections.getRow(request.params.id);
    return app.ctx.connections.listAllSchedulers(row, {
      ...(query.q !== undefined ? { q: query.q } : {}),
      ...(query.withinMs !== undefined ? { withinMs: query.withinMs } : {}),
      limit: query.limit,
    });
  });
