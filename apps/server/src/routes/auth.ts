...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  app.post("/auth/login", async (request, reply): Promise<MeResponse> => {
     ...
     */
    /**
     * Checked after the password for the same reason as "require SSO" below:
     * a wrong password on a disabled account must look exactly like a wrong
     * password on any other. The right password on a disabled account gets the
     * honest answer.
     */
    if (row.disabledAt) {
      request.auditTarget({ action: "auth.login_failed" });
      request.auditDetail({ email: input.email, reason: "disabled" });
      request.log.info({ email: input.email }, "login refused: account disabled");
      throw unauthenticated("This account has been disabled. Ask an admin to re-enable it.");
    }
