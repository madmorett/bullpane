...
export async function queueRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<QueueParams>(`${base}/pause`, { preHandler: [operator] }, async (request) => {
    const input = pauseQueueSchema.parse(request.body ?? {});
    const inspector = await app.ctx.connections.getInspector(request.params.id);
    await withRedis(() => inspector.pauseQueue(request.params.queue));
    // The reason lives in the audit row, not in Redis: it answers "why was this
    // paused" months later, which is the audit log's job and nobody else's.
    if (input.reason) request.auditDetail({ reason: input.reason });
    request.log.info({ queue: request.params.queue, by: request.user?.id, hasReason: !!input.reason }, "queue paused");
    return { ok: true };
  });
