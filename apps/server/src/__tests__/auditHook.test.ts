...
describe("audit hook on the real app", () => {
  ...
  it("records a queue pause with the action derived from the route", async () => {
    ...
    // No reason given: no detail invented for it.
    expect(row.detail).toBeNull();
  });

  it("keeps the operator's reason for a pause in the audit detail", async () => {
    const res = await world.app.inject({
      method: "POST",
      url: "/api/connections/c1/queues/payments/pause",
      payload: { reason: "  payment provider degraded, holding refunds  " },
    });
    expect(res.statusCode).toBe(200);
    await close(world.app);
    const row = rows()[0]!;
    expect(row.action).toBe("queue.pause");
    // Trimmed, and nowhere near Redis: the paused flag has no room for it and
    // "why was this paused" is a question for the audit log.
    expect(row.detail).toEqual({ reason: "payment provider degraded, holding refunds" });
  });
