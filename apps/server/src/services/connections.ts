...
/**
 * How many queues are read for schedulers at once. Same reasoning as the
 * inspector's BULK_CONCURRENCY: a small window finishes in about the same
 * wall-clock time without queueing commands ahead of the customer's workload.
 */
const SCHEDULER_FANOUT_CONCURRENCY = 6;
/** Per-queue ceiling on schedulers read during the connection-wide fan-out. */
const SCHEDULERS_PER_QUEUE_CAP = 500;

/** Substring match over the fields a user would actually search by. */
function schedulerMatches(s: { key: string; name: string }, queueName: string, needle: string): boolean {
  return (
    s.key.toLowerCase().includes(needle) ||
    s.name.toLowerCase().includes(needle) ||
    queueName.toLowerCase().includes(needle)
  );
}
...
export class ConnectionsService {
  ...
  async listRows(): Promise<ConnectionRow[]> {
    // `position` is the manual order; `createdAt` only breaks ties so that rows
    // sharing a position (an older install, a half-applied reorder) still come
    // back in a stable order instead of whatever MySQL feels like.
    return this.db.select().from(connections).orderBy(connections.position, connections.createdAt);
  }
  ...
  async create(input: CreateConnectionInput): Promise<RedisConnection> {
    const id = nanoid();
    // A new connection lands at the bottom of the sidebar rather than jumping
    // into the middle of an order somebody arranged on purpose.
    const siblings = await this.db.select({ id: connections.id }).from(connections);
  ...
  /**
   * Rewrite the sidebar order from a full list of ids.
   *
   * WHY THE WHOLE LIST AND NOT ONE POSITION: a drag is a single user action, and
   * sending it as N per-row updates means a half-applied order survives a failure
   * halfway through. Renumbering everything from the array index also makes
   * duplicate or gapped positions self-healing — whatever the table looked like
   * before, it comes out 0..n-1.
   *
   * Ids not present in `ids` keep their rows but are pushed to the end, so a
   * connection somebody else adds mid-drag cannot be silently dropped from view.
   */
  async reorder(ids: string[]): Promise<RedisConnection[]> {
    const rows = await this.listRows();
    const known = new Set(rows.map((r) => r.id));
    for (const id of ids) {
      if (!known.has(id)) throw notFound("Connection");
    }
    // Anything the client did not mention keeps its relative order, after the
    // ids it did send.
    const ordered = [...ids, ...rows.filter((r) => !ids.includes(r.id)).map((r) => r.id)];
    await this.db.transaction(async (tx) => {
      for (const [index, id] of ordered.entries()) {
        await tx.update(connections).set({ position: index }).where(eq(connections.id, id));
      }
    });
    return this.list();
  }
  ...
  /**
   * Every job scheduler of a connection, in one call.
   *
   * The fan-out is driven by `schedulersCount` from the stats read the sidebar
   * already makes: a queue that reports 0 costs no scheduler read at all, so on a
   * Redis with 200 queues and 3 schedulers this is 3 reads, not 200. Queues are
   * read with bounded concurrency rather than a Promise.all so a big install does
   * not fire 50 concurrent EVALSHAs at the customer's Redis.
   *
   * A queue whose read fails lands in `failed` instead of throwing: one broken
   * queue must not blank the whole page.
   */
  async listAllSchedulers(
    row: ConnectionRow,
    opts: { q?: string; withinMs?: number; limit: number },
  ): Promise<ConnectionSchedulersPage> {
    const summaries = await this.listQueues(row);
    const withSchedulers = summaries.filter((s) => s.schedulersCount > 0);
    const inspector = this.inspectorFor(row);

    const schedulers: ConnectionScheduler[] = [];
    const failed: { queueName: string; error: string }[] = [];
    let total = 0;

    const cutoff = opts.withinMs === undefined ? null : Date.now() + opts.withinMs;
    const needle = opts.q?.trim().toLowerCase() || null;

    const queue = [...withSchedulers];
    const worker = async (): Promise<void> => {
      for (;;) {
        const summary = queue.shift();
        if (!summary) return;
        try {
          // Page the whole `repeat` zset of this queue, capped: schedulersCount is
          // authoritative and is almost always small (tens), but a pathological
          // queue must not be able to pull a million rows into memory.
          const end = Math.min(summary.schedulersCount, SCHEDULERS_PER_QUEUE_CAP) - 1;
          const page = await withRedis(() => inspector.getSchedulers(summary.name, { start: 0, end }));
          total += page.total;
          for (const s of page.schedulers) {
            if (cutoff !== null && (s.next === null || s.next > cutoff)) continue;
            if (needle && !schedulerMatches(s, summary.name, needle)) continue;
            schedulers.push({ ...s, queueName: summary.name });
          }
        } catch (err) {
          failed.push({ queueName: summary.name, error: err instanceof Error ? err.message : String(err) });
        }
      }
    };

    await Promise.all(Array.from({ length: Math.min(SCHEDULER_FANOUT_CONCURRENCY, queue.length) }, worker));

    // Soonest first: "what fires next" is the question this page exists to answer.
    // Schedulers with no next run (ended / limit reached) sink to the bottom.
    schedulers.sort((a, b) => {
      if (a.next === null && b.next === null) return a.queueName.localeCompare(b.queueName);
    ...
    return {
      schedulers: schedulers.slice(0, opts.limit),
      total,
      queuesScanned: summaries.length,
      queuesWithSchedulers: withSchedulers.length,
      failed,
    };
  }
