...
/** root of the three-level flow built in beforeAll, used by the tree walk tests */
let treeRoot: Awaited<ReturnType<FlowProducer["add"]>>;
const queues: Queue[] = [];
...
beforeAll(async () => {
  ...
  // A three-level flow across two queues, for the tree walk: the root waits on two
  // children, and one of those children waits on a grandchild of its own.
  treeRoot = await flow.add({
    name: "invoice",
    queueName: "tree-root",
    data: { kind: "invoice" },
    children: [
      {
        ...
        queueName: "tree-mid",
        data: { part: "totals" },
        children: [{ name: "fetch-rates", queueName: "tree-leaf", data: { part: "rates" } }],
      },
      { name: "notify", queueName: "tree-leaf", data: { part: "notify" } },
    ],
  });
  await flow.close();
...
describe("connection", () => {
  ...
  it("fails fast on a dead redis", async () => {
    // A port nothing listens on, found at runtime: a fixed one (6398) was taken by
    // an unrelated tunnel on a dev machine and turned this into a false failure.
    ...
    const dead = new RedisInspector({ id: "dead", url: `redis://127.0.0.1:${port}` }, { connectTimeoutMs: 500 });
...
describe("discovery", () => {
  it("finds every queue through SCAN on meta keys", async () => {
    ...
    expect(names).toEqual(["emails", "orders", "proq", "proq2", "reports", "tree-leaf", "tree-mid", "tree-root"]);
  });
...
describe("getJobs", () => {
  ...
  it("reads zset states with score ordering", async () => {
    ...
    // delayedUntil is decoded from the zset score, not from the hash
    for (const j of delayed.jobs) {
      expect(j.delayedUntil).not.toBeNull();
      expect(Math.abs(j.delayedUntil! - (j.timestamp + 60_000))).toBeLessThan(5);
    }
    const detail = await inspector.getJob("orders", delayed.jobs[0].id);
    expect(detail!.delayedUntil).toBe(delayed.jobs[0].delayedUntil);
    const prio = await inspector.getJobs("orders", "prioritized", { start: 0, end: 10, order: "asc" });
    expect(prio.jobs[0].priority).toBe(5);
    expect(prio.jobs[0].delayedUntil).toBeNull();
    expect(failed.jobs[0].delayedUntil).toBeNull();
  });
});
...
describe("getJobTree (parent/child of one flow instance)", () => {
  it("walks the whole tree from the root, across queues", async () => {
    const tree = await inspector.getJobTree("tree-root", treeRoot.job.id!);
    expect(tree).not.toBeNull();
    expect(tree!.truncated).toBe(false);
    // root + 2 children + 1 grandchild
    expect(tree!.nodes).toHaveLength(4);
    expect(tree!.rootKey).toBe(`bull:tree-root:${treeRoot.job.id}`);

    const byName = new Map(tree!.nodes.map((n) => [n.name, n]));
    expect([...byName.keys()].sort()).toEqual(["fetch-rates", "invoice", "notify", "totals"]);

    // the walk crosses queues, and each node reports the queue it really lives in
    expect(byName.get("invoice")!.queueName).toBe("tree-root");
    expect(byName.get("totals")!.queueName).toBe("tree-mid");
    expect(byName.get("notify")!.queueName).toBe("tree-leaf");
    expect(byName.get("fetch-rates")!.queueName).toBe("tree-leaf");

    // edges point child -> parent and match the shape we asked FlowProducer for
    expect(byName.get("invoice")!.parentKey).toBeNull();
    expect(byName.get("totals")!.parentKey).toBe(`bull:tree-root:${treeRoot.job.id}`);
    expect(byName.get("fetch-rates")!.parentKey).toBe(byName.get("totals")!.key);
  });

  it("reports dependency counts and the waiting-children state of parents", async () => {
    const tree = await inspector.getJobTree("tree-root", treeRoot.job.id!);
    const byName = new Map(tree!.nodes.map((n) => [n.name, n]));

    // nothing has run: the root waits on both children, `totals` on its one child
    expect(byName.get("invoice")!.dependencies).toEqual({ processed: 0, unprocessed: 2 });
    expect(byName.get("totals")!.dependencies).toEqual({ processed: 0, unprocessed: 1 });
    expect(byName.get("invoice")!.state).toBe("waiting-children");
    expect(byName.get("totals")!.state).toBe("waiting-children");
    // leaves have no children of their own
    expect(byName.get("fetch-rates")!.dependencies).toBeNull();
    expect(byName.get("notify")!.dependencies).toBeNull();
  });

  it("climbs to the root when asked about a deep child, and does not when told not to", async () => {
    const tree = await inspector.getJobTree("tree-root", treeRoot.job.id!);
    const grandchild = tree!.nodes.find((n) => n.name === "fetch-rates")!;

    const climbed = await inspector.getJobTree(grandchild.queueName, grandchild.id);
    expect(climbed!.rootKey).toBe(`bull:tree-root:${treeRoot.job.id}`);
    expect(climbed!.focusKey).toBe(grandchild.key);
    expect(climbed!.climbedLevels).toBe(2);
    expect(climbed!.nodes).toHaveLength(4);

    // fromRoot:false starts where it was asked, so it only sees itself
    const local = await inspector.getJobTree(grandchild.queueName, grandchild.id, { fromRoot: false });
    expect(local!.climbedLevels).toBe(0);
    expect(local!.rootKey).toBe(grandchild.key);
    expect(local!.nodes).toHaveLength(1);
  });

  it("honours the node budget instead of returning an unbounded tree", async () => {
    const tree = await inspector.getJobTree("tree-root", treeRoot.job.id!, { maxNodes: 2 });
    expect(tree!.truncated).toBe(true);
    expect(tree!.nodes.length).toBeLessThanOrEqual(2);
    // breadth-first, so the budget keeps the TOP of the tree, never a random slice
    expect(tree!.nodes[0]!.name).toBe("invoice");
  });

  it("returns null for a job that does not exist", async () => {
    expect(await inspector.getJobTree("tree-root", "999999")).toBeNull();
  });

  it("returns a single node for a job that is not part of any flow", async () => {
    const tree = await inspector.getJobTree("orders", "1");
    expect(tree!.nodes).toHaveLength(1);
    expect(tree!.nodes[0]!.parentKey).toBeNull();
    expect(tree!.nodes[0]!.dependencies).toBeNull();
  });
});
...
describe("success rate vs retention (removeOnComplete)", () => {
  ...
  it("uses BullMQ's metrics and ignores the zset pruning", async () => {
    const queue = q("retention-with-metrics");
    const w = new Worker(
      "retention-with-metrics",
      async (job: Job) => {
    ...
    for (let i = 0; i < 200; i++) await queue.add("ok", { i }, { removeOnComplete: { count: 50 }, attempts: 1 });
    for (let i = 0; i < 20; i++) await queue.add("bad", { i }, { removeOnComplete: { count: 50 }, attempts: 1 });
    await waitFor(async () => {
    ...
    const s = (await inspector.getQueueStats(["retention-with-metrics"]))["retention-with-metrics"];
  ...
  it("flags retentionSkewed when there are no metrics and the queue prunes completed jobs", async () => {
    const queue = q("retention-without-metrics");
    const w = new Worker(
    ...
    for (let i = 0; i < 60; i++) await queue.add("ok", { i }, { removeOnComplete: { count: 10 }, attempts: 1 });
    for (let i = 0; i < 5; i++) await queue.add("bad", { i }, { removeOnComplete: { count: 10 }, attempts: 1 });
    await waitFor(async () => {
      const c = await queue.getJobCounts("completed", "failed");
    ...
    const s = (await inspector.getQueueStats(["retention-without-metrics"]))["retention-without-metrics"];
  ...
  it("does not flag skew when the queue keeps everything", async () => {
    const queue = q("retention-keeps-all");
    const w = new Worker(
      "retention-keeps-all",
      async (job: Job) => {
        if (job.name === "bad") throw new Error("failed");
    ...
    workers.push(w);
    for (let i = 0; i < 8; i++) await queue.add("ok", { i }, { attempts: 1 });
    for (let i = 0; i < 2; i++) await queue.add("bad", { i }, { attempts: 1 });
    await waitFor(async () => {
      const c = await queue.getJobCounts("completed", "failed");
    ...
    const s = (await inspector.getQueueStats(["retention-keeps-all"]))["retention-keeps-all"];
    expect(s.rates.source).toBe("zset");
    expect(s.rates.retentionSkewed).toBe(false);
...
describe("getMetricsCounters (source of the error alerts)", () => {
  it("returns the cumulative counters of a queue that collects metrics", async () => {
    ...
    workers.push(w);
    for (let i = 0; i < 12; i++) await queue.add("ok", { i }, { attempts: 1 });
    for (let i = 0; i < 3; i++) await queue.add("bad", { i }, { attempts: 1 });
    await waitFor(async () => {
      const c = await queue.getJobCounts("completed", "failed");
  ...
  it("returns null (not zero) when the queue does NOT collect metrics", async () => {
    ...
    const NAME = "counters-without-metrics";
    const queue = q(NAME);
    const w = new Worker(
      NAME,
      async (job: Job) => {
    ...
    workers.push(w);
    for (let i = 0; i < 5; i++) await queue.add("ok", { i }, { attempts: 1 });
    for (let i = 0; i < 2; i++) await queue.add("bad", { i }, { attempts: 1 });
    await waitFor(async () => {
      const c = await queue.getJobCounts("completed", "failed");
  ...
  it("is right where the ZCOUNT would be wrong: aggressive pruning of completed jobs", async () => {
    ...
    for (let i = 0; i < 300; i++) await queue.add("ok", { i }, { removeOnComplete: { count: 50 }, attempts: 1 });
    for (let i = 0; i < 15; i++) await queue.add("bad", { i }, { removeOnComplete: { count: 50 }, attempts: 1 });
    await waitFor(async () => {
      const c = await queue.getJobCounts("completed", "failed");
  ...
  it("the counter is already valid before the minute rolls over, while the :data list is still empty", async () => {
    ...
    workers.push(w);
    for (let i = 0; i < 4; i++) await queue.add("ok", { i }, { attempts: 1 });
    await waitFor(async () => (await queue.getJobCounts("completed")).completed === 4);
    await w.close();
...
describe("job schedulers (repeatable jobs)", () => {
  ...
  it("pages by start/end and orders by next run", async () => {
    const queue = q(NAME);
    await queue.upsertJobScheduler("z-later", { pattern: "0 4 * * *" });
