...
import { Job, type JobsOptions, Queue, UnrecoverableError } from "bullmq";
import {
  EMPTY_COUNTS,
  STATE_KEY,
  type BulkJobAction,
...
export class RedisInspector implements Inspector {
  ...
  private rowToSummary(row: LuaReply, state: JobState | "unknown"): JobSummary {
    const { id, hash, truncated, dataBytes, score } = rowToHash(asArray(row));
    return hashToSummary(this.config.prefix, id, hash, state, truncated, dataBytes, score);
  }
  ...
  /**
   * Walk ONE flow instance: the tree of real jobs hanging off a parent.
   *
   * Cost model, because this is the read most likely to get expensive:
   *  - Every job is read exactly once, by `getTreeNode.lua` (HMGET of 10 fields +
   *    a few O(1) state probes + bounded SSCAN/HSCAN). No HGETALL, no logs.
   *  - Nodes are expanded in BREADTH-FIRST waves, and each wave is grouped BY QUEUE
   *    and issued as one pipeline per queue. So a 200-node tree spread over 3 queues
   *    costs ~3 round trips per level, not 200.
   *  - Every pipeline touches keys of a single queue, which is what keeps this
   *    cluster safe: a child in another queue lands in that queue's own pipeline.
   *  - `maxNodes` is a hard budget on jobs READ, not just returned. Hitting it sets
   *    `truncated` and stops the walk; a fan-out parent with 50k children returns
   *    the first slice with childrenTruncated set, never 50k nodes.
   *
   * Climbing to the root first (`fromRoot`) is bounded by MAX_TREE_CLIMB so a
   * corrupted parent chain (or a cycle) cannot loop forever.
   */
  async getJobTree(
    queueName: string,
    jobId: string,
    ...
    const focusKey = `${queueKey(prefix, queueName)}:${jobId}`;
    let visited = 0;

    /** Read one job by its full key (`${prefix}:${queue}:${id}`). */
    const readOne = async (fullKey: string): Promise<RawTreeNode | null> => {
      const ref = splitJobKey(prefix, fullKey);
      if (!ref) return null;
      const replies = await this.readTreeNodes(c, ref.queueName, [ref.id], maxNodes);
      visited += 1;
      return replies[0] ?? null;
    };

    const focus = await readOne(focusKey);
    ...
    if (opts.fromRoot !== false) {
      ...
      let current = focus;
      while (current.parentKey && climbedLevels < MAX_TREE_CLIMB) {
        const next: string = current.parentKey;
        if (seen.has(next)) break; // cycle: stop where we are rather than spin
        const parent = await readOne(next);
    ...
    // 2) breadth-first down from the root, one pipeline per queue per wave
    const nodes = new Map<string, JobTreeNode>();
    const rootRef = splitJobKey(prefix, rootKey);
    if (!rootRef) return null;

    let frontier: { key: string; parentKey: string | null }[] = [{ key: rootKey, parentKey: null }];
    let truncated = false;

    while (frontier.length > 0 && !truncated) {
      // group this wave by queue: one pipeline per queue keeps every call
      // single-slot on a cluster and turns a wide level into a few round trips.
      const byQueue = new Map<string, { id: string; key: string; parentKey: string | null }[]>();
      for (const item of frontier) {
        if (nodes.has(item.key)) continue;
        const ref = splitJobKey(prefix, item.key);
        if (!ref) continue;
        const bucket = byQueue.get(ref.queueName);
        const entry = { id: ref.id, key: item.key, parentKey: item.parentKey };
        if (bucket) bucket.push(entry);
        else byQueue.set(ref.queueName, [entry]);
      }

      const next: { key: string; parentKey: string | null }[] = [];

      for (const [q, entries] of byQueue) {
        // respect the budget before spending a round trip on the wave
        const room = maxNodes - nodes.size;
        if (room <= 0) {
          truncated = true;
          break;
        }
        const slice = entries.length > room ? (truncated = true, entries.slice(0, room)) : entries;

        const replies = await this.readTreeNodes(
          c,
          q,
          slice.map((e) => e.id),
          maxNodes,
        ...
        for (let i = 0; i < slice.length; i += 1) {
          ...
          const raw = replies[i];
          if (!raw) {
            // referenced by a parent's set but the hash is gone (cleaned/removed)
            nodes.set(entry.key, missingTreeNode(prefix, entry.key, entry.parentKey));
            continue;
          }
          nodes.set(entry.key, rawToTreeNode(prefix, entry.key, entry.parentKey, raw));
          for (const childKey of raw.children) {
            if (!nodes.has(childKey)) next.push({ key: childKey, parentKey: entry.key });
          }
        }
      }
  ...
  /**
   * Read N jobs of the SAME queue in one pipeline. Single queue by construction,
   * so the whole pipeline lands on one cluster slot.
   */
  private async readTreeNodes(c: RedisClient, queueName: string, ids: string[], maxNodes: number): Promise<(RawTreeNode | null)[]> {
    if (ids.length === 0) return [];
    const p = queueKeyPrefix(this.config.prefix, queueName);
    const stateKeys = allStateKeys(this.config.prefix, queueName);
    const pipeline = c.pipeline();
    for (const id of ids) {
      pipelineScript(pipeline, "getTreeNode", [
        p + JOB_KEY.hash(id),
        p + JOB_KEY.dependencies(id),
        p + JOB_KEY.processed(id),
        ...stateKeys,
        id,
        // never ask Redis for more children than the whole walk could hold
        maxNodes,
      ]);
    }
    const results = await pipeline.exec();
    return ids.map((_, i) => {
      const entry = results?.[i];
      if (!entry || entry[0]) return null; // a per-job error must not kill the tree
      return parseTreeNode(entry[1] as LuaReply);
    });
  }
  ...
  /**
   * Operator "discard": move an active job to failed with no retry. An
   * UnrecoverableError is what bullmq's own moveToFailed treats as "do not
   * retry, whatever opts.attempts says" (Job#discard did the same and was
   * removed in bullmq 6). The lock token "0" makes moveToFinished skip the
...
/**
 * How far up a parent chain getJobTree will climb before giving up. A flow deeper
 * than this is pathological; the cap also makes a corrupted `parentKey` chain
 * terminate instead of looping.
 */
const MAX_TREE_CLIMB = 50;

/**
 * `bull:orders:42` -> { queueName: "orders", id: "42" }.
 *
 * Split on the LAST colon: the job id is the final segment, and everything before
 * it is the qualified queue name (which may itself contain colons, e.g. a cluster
 * hash tag or a prefix like `bull:tenant-a`).
 */
function splitJobKey(prefix: string, fullKey: string): { queueName: string; id: string } | null {
  const cut = fullKey.lastIndexOf(":");
  if (cut <= 0) return null;
  const qualified = fullKey.slice(0, cut);
  const id = fullKey.slice(cut + 1);
  if (!id) return null;
  return { queueName: queueNameFromQueueKey(prefix, qualified), id };
}

function rawToTreeNode(prefix: string, key: string, parentKey: string | null, raw: RawTreeNode): JobTreeNode {
  const ref = splitJobKey(prefix, key);
  const queueName = ref?.queueName ?? "";
  return {
    key,
    id: ref?.id ?? "",
    queueName,
    queueKey: queueKey(prefix, queueName),
    name: raw.name ?? "",
    state: isJobState(raw.state) ? raw.state : "unknown",
    timestamp: raw.timestamp,
    finishedOn: raw.finishedOn,
    attemptsMade: raw.attemptsMade,
    failedReason: raw.failedReason,
    progress: raw.progress,
    // trust the walk's own edge over the job's stored parentKey: the walk knows
    // which parent it arrived from, which is what the drawn tree must match.
    parentKey: parentKey ?? raw.parentKey,
    dependencies: raw.processed + raw.unprocessed > 0 ? { processed: raw.processed, unprocessed: raw.unprocessed } : null,
    childrenTruncated: raw.childrenTruncated,
    missing: false,
  };
}

/** A child a parent still references whose hash is gone (removed or cleaned). */
function missingTreeNode(prefix: string, key: string, parentKey: string | null): JobTreeNode {
  const ref = splitJobKey(prefix, key);
  const queueName = ref?.queueName ?? "";
  return {
    key,
